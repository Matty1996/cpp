...
void displayBoth12And24HourTimeFormats(int hours, int minutes, int seconds) { //function for the display of both clocks
    ...
    cout << "12-hour format: " << outputTime12HourTimeFormat(hours, minutes, seconds) << endl; // 12 hour format set to instructions
    cout << "24-hour format: " << outputTime24HourTimeFormat(hours, minutes, seconds) << endl; // 24 hour format set to instructions 
}
...
int main() {
    ...
    do { //we need a do while loop so we can ensure the clock works correctly and doesnt excede how many seconds are in a minute and how many minues are in an hour and how many hours there are in am and pm
